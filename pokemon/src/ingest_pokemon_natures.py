...
tabela_destino = f"{used_catalog}.{used_schema}.raw_pokemon_natures"
...
ctrl_id = spark.sql(
    f"SELECT raw_pokemon_natures FROM {tabela_controle} WHERE id = 1"
).collect()
...
API_URL = "https://pokeapi.co/api/v2/nature?limit=2000"
all_rows = []
...
if all_rows:
    df = spark.createDataFrame(all_rows, ["id_nature", "name"])
    display(df)
    ...
    spark.sql(f"""
        ...
        SET raw_pokemon_natures = {ultimo_id_ingestao}
        WHERE id = 1
    """)
