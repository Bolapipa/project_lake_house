...
tabela_destino = f"{used_catalog}.{used_schema}.raw_items_attributes"
...
while API_URL:
    ...
    for attr in dados.get("results", []):
        ...
        all_rows_items_attributes.append(row_attr)

    API_URL = dados.get("next")

print(f"Total de item-attributes retornados: {len(all_rows_items_attributes)}")

# COMMAND ----------

if all_rows_items_attributes:
    df_attr = spark.createDataFrame(
        all_rows_items_attributes,
        ["raw_items_attributes", "name"]
    )
    ...
    ultimo_id_ingestao = all_rows_items_attributes[-1][0]

    spark.sql(f"""
        UPDATE {tabela_controle}
        SET raw_items_attributes = {ultimo_id_ingestao}
        WHERE id = 1
    """)
