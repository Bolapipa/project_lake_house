...
tabela_destino = f"{used_catalog}.{used_schema}.raw_pokemon_location_areas"
...
ctrl_id = spark.sql(
    f"SELECT raw_pokemon_location_areas FROM {tabela_controle} WHERE id = 1"
).collect()
...
API_URL = "https://pokeapi.co/api/v2/location-area?limit=20"
all_rows = []
...
if all_rows:
    df = spark.createDataFrame(all_rows, ["id_location_area", "name"])
    display(df)
    ...
    spark.sql(f"""
        ...
        SET raw_pokemon_location_areas = {ultimo_id_ingestao}
        WHERE id = 1
    """)
