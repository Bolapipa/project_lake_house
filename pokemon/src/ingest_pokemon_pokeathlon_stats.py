...
tabela_destino = f"{used_catalog}.{used_schema}.raw_pokemon_pokeathlon_stats"
...
ctrl_id = spark.sql(
    f"SELECT raw_pokemon_pokeathlon_stats FROM {tabela_controle} WHERE id = 1"
).collect()
...
API_URL = "https://pokeapi.co/api/v2/pokeathlon-stat?limit=20"
all_rows = []
...
if all_rows:
    df = spark.createDataFrame(all_rows, ["id_pokeathlon_stat", "name"])
    display(df)
    ...
    spark.sql(f"""
        ...
        SET raw_pokemon_pokeathlon_stats = {ultimo_id_ingestao}
        WHERE id = 1
    """)
