...
tabela_destino = f"{used_catalog}.{used_schema}.raw_pokemon_growth_rates"
...
ctrl_id = spark.sql(
    f"SELECT raw_pokemon_growth_rates FROM {tabela_controle} WHERE id = 1"
).collect()
...
API_URL = "https://pokeapi.co/api/v2/growth-rate?limit=2000"
all_rows = []
...
if all_rows:
    df = spark.createDataFrame(all_rows, ["id_growth_rate", "name"])
    display(df)
    ...
    spark.sql(f"""
        ...
        SET raw_pokemon_growth_rates = {ultimo_id_ingestao}
        WHERE id = 1
    """)
