...
(
  ...
  timezone_continente STRING COMMENT 'Continente do fuso horário (ex: America, Europe).',
  timezone_estado STRING COMMENT 'Estado/Região do fuso horário (ex: Sao_Paulo, Fortaleza).',
...
SELECT
  ...
  SPLIT(trim(timezone), '/')[0] AS timezone_continente,
  SPLIT(trim(timezone), '/')[1] AS timezone_estado,
