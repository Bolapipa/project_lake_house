...
DAG_ID = "open_meteo_databricks_prod"
TASK_ID = "executar_job_open_meteo_prod"
DEFAULT_JOB_ID = 47357426867496
...
DATABRICKS_JOB_ID = _read_int_env("OPEN_METEO_PROD_JOB_ID", DEFAULT_JOB_ID)
DATABRICKS_CONN_ID = os.getenv("OPEN_METEO_PROD_CONN_ID", DEFAULT_CONN_ID)
DAG_SCHEDULE = os.getenv("OPEN_METEO_PROD_SCHEDULE", DEFAULT_SCHEDULE) or DEFAULT_SCHEDULE
...
with DAG(
    ...
    tags=["open_meteo", "databricks", "prod"],
) as dag:
    # Dispara o job do Databricks em prod com politica de retry no Airflow e no hook Databricks.
    executar_job_open_meteo_prod = DatabricksRunNowOperator(
        task_id=TASK_ID,
